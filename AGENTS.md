...
GridInfluence is a standalone .NET library for sparse tiled integer influence fields. It has no
dependency on any timeline package and must not gain one. There is exactly one library project:
`src/GridInfluence`.
...
- The warm data path is unmanaged: no managed arrays, LINQ, delegates, boxing, reflection, locks,
  or exceptions on process or query paths.
...
- Correctness receipts precede timing. Benchmarks consume success, playback, and output state;
  warm process and query paths must allocate 0 B.
...
Package changes also require `dotnet pack` plus a version review. Never move a published tag.
